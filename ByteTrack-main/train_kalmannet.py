...
def train():
    ...
    print("Start Training KalmanNet (Branch Fusion F1+F2+F4+Conf) ...")
    ...
    for epoch in range(epochs):
        ...
        for b_obs, b_gt in dataloader:
            ...
            for t in range(1, seq_len):
                ...
                batch_loss = batch_loss + loss_pos + 2.0 * loss_vel + 0.05 * loss_reg
        ...
        with torch.no_grad():
            for b_obs, b_gt in val_dataloader:
                ...
                for t in range(1, seq_len):
                    ...
                    loss_vel = criterion(current_state[:, 4:8], gt_vel)
                    loss_reg = torch.mean(delta_k.pow(2))
                    val_loss += (loss_pos + 2.0 * loss_vel + 0.05 * loss_reg).item()
